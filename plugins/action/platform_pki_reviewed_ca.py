"""Install a reviewed validation CA from a pinned controller descriptor."""
...
MAX_REVIEWED_CA_SIZE = 1024 * 1024
...
def pin_source(path: str, expected_digest: str) -> PinnedSource:
    ...
    return pin_controller_source(
        path,
        expected_digest,
        maximum=MAX_REVIEWED_CA_SIZE,
        label="reviewed validation CA",
    )
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if set(self._task.args) != {"source", "sha256", "dest", "mode"}:
            raise AnsibleActionFail(
                "reviewed CA action accepts only source, digest, destination, and mode"
            )
        ...
        if (
            ...
            raise AnsibleActionFail(
                "reviewed CA destination must be an absolute canonical path"
            )
