...
assert_frontend_tls_rejected() {
  ...
  http_code="$(podman exec "$CONTAINER" curl --silent --show-error --noproxy '*' \
    ...
    --cacert "$TEST_DIR/server-ca.crt" \
    "${certificate_args[@]}" \
    --resolve loki.test.invalid:8443:127.0.0.1 \
    -H 'Host: loki.test.invalid' \
    --output /dev/null --write-out '%{http_code}' \
    https://loki.test.invalid:8443/loki/api/v1/query_range 2>/dev/null)"
  status=$?
  set -e
...
for fixture in loki grafana wrong untrusted patroni; do
...
for fixture in loki grafana patroni; do
  for _ in {1..30}; do
