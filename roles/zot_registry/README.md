...
Default behavior listens on host port `443/tcp`, forwards to Zot's container port `5000`, stores registry blobs under `/var/lib/zot/data`, keeps the Zot UI disabled, and uses `zot_registry_tls_custody: managed`. Managed custody retains the controller-source behavior and requires `zot_registry_tls_cert_src` and `zot_registry_tls_key_src` when TLS is enabled.

Set `zot_registry_tls_custody: host-local` only after the host-local lifecycle has activated and authenticated a version, and keep `zot_registry_tls_enabled: true`; the role rejects host-local custody when TLS is disabled. The role invokes the fixed `/usr/local/libexec/platform-pki-host-local-lifecycle active-paths` interface on the target and renders only its authenticated `fullchain.crt` and `tls.key` result. Inventory does not supply certificate or key paths. The helper must be an installed root-owned, group-root, non-symlink regular file with mode `0755`; unresolved lifecycle state or an absent active version makes its read-only lookup fail closed. The lifecycle helper remains the sole owner of host-local Zot configuration changes: normal role convergence previews the rendered configuration and refuses drift rather than invalidating the authenticated active record.

Host-local lookup inputs are `zot_registry_tls_host_local_state_root`, `zot_registry_tls_host_local_pending_root`, `zot_registry_tls_host_local_versions_root`, `zot_registry_tls_host_local_service`, `zot_registry_tls_host_local_target`, and `zot_registry_tls_host_local_zot_config_path`. The helper and Zot config paths are fixed. For `registry-dev`, the roots are fixed to `/var/lib/platform-config/pki/host-local/registry-dev`, `/etc/zot/tls-pending`, and `/etc/zot/tls-versions`; set the target to the exact inventory hostname. Zot serves the authenticated full chain so strict external validation can observe the intermediate certificate. Host-local custody never copies controller TLS files.
