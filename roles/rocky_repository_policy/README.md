...
nonempty mapping keyed by lowercase repository ID. Every entry must contain
...
`metalink`, or `mirrorlist`. Package origins must be credential-free HTTPS,
package signature checking must be enabled, and signing keys must use absolute
local `file://` URLs. Lists are compared without regard to order; IDs and all six
effective fields must otherwise match exactly. Failure output identifies a
release mismatch or differing IDs or field names without printing private URL
values.
